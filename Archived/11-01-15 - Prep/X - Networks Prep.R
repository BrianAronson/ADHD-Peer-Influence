...
#School 77
        ...
        g1<-simplify(g)
        S77W1<-get.adjacency(g1,sparse=FALSE)
        #S77W2
        #Create number, starting with 1, for each unique sender value
        ...
        g2=graph.data.frame(Friends77M23, directed=TRUE)
        g2<-simplify(g2)
        S77W2<-get.adjacency(g2,sparse=FALSE)
        ...
        g0=graph.data.frame(Friends77M03, directed=TRUE)
        g0<-simplify(g0)
        S77W0<-get.adjacency(g0,sparse=FALSE)        
        
        
#Put NAs for people missing from wave        
#        for(i in 1:nrow(S58W1)){
#          ifelse(ADHDFriends58M1$Missing[i]==1, apply(S58W1,i, function(x) x<-NA),"")
#        }
        
#        S58W1T1<-cbind(S58W1,ADHDFriends58M1$Missing)
#        S58W1T2<-rbind(S58W1,ADHDFriends58M1$Missing)

#        as.matrix(ifelse(ADHDFriends58M1$Missing==0,S58W1,NA))
        
        
        
#        S58W1T1<-cbind(S58W1,ADHDFriends58M1$Missing) #Add missing info
 #       S58W1T1[S58W1T1[, 973]==1] <- NA  #drop rows that are missing
  #      S58W1<-S58W1T1[,-973]  #remove added column
   #     S58W1T2<-rbind(S58W1,ADHDFriends58M1$Missing)
    #    S58W1T2<-S58W1T2[973, ]==1 <- NA  #drop columns that are missing
     #   for (i in 1:972) {
      #    if (S58W1T2[973,i]==1){
       #     S58W1T2[S58W1T2[,i]] <- NA
        #  }
        #}
...
#        } S58W1T2[973, ]==1 <- NA  #drop columns that are missing
#        S58W1<-S58W1T2[-973,]  #remove added row
          
#        S58W0T1<-cbind(S58W0,ADHDFriends58M0$Missing) #Add missing info
#        S58W0T1[S58W0T1[, 973]==1] <- NA  #drop rows that are missing
#        S58W0<-S58W0T1[,-973]  #remove added column
#        S58W0T2<-rbind(S58W0,ADHDFriends58M0$Missing)
#        S58W0T2[S58W0T2[973, ]==1] <- NA  #drop columns that are missing
#        S58W0<-S58W0T2[-973,]  #remove added column
        
#        S58W2T1<-cbind(S58W2,ADHDFriends58M2$Missing) #Add missing info
#        S58W2T1[S58W2T1[, 973]==1] <- NA  #drop rows that are missing
#        S58W2<-S58W2T1[,-973]  #remove added column
#        S58W2T2<-rbind(S58W2,ADHDFriends58M2$Missing)
#        S58W2T2[S58W2T2[973, ]==1] <- NA  #drop rows that are missing
#        S58W2<-S58W2T2[-973,]  #remove added column
        
        
       table(degree(g0, v = V(g0), mode = c("out"),
