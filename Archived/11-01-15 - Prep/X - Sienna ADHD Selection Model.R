...
#ADHD Homophily Sienna Model 
  ADHD1<-data.frame(V1=ADHDFriends58M0$ADHD, V2=ADHDFriends58M1$ADHD, V3=ADHDFriends58M2$ADHD)
  ADHD1[is.na(ADHD1)]<-2
  ADHD1$ADHD<-ifelse(ADHD1$V1==1|ADHD1$V2==1|ADHD1$V3==1,1,ifelse(ADHD1$V1==0|ADHD1$V2==0|ADHD1$V3==0,0,NA))

  FriendsArrayT1 <- array( c( S58W0, S58W1,S58W2),
                         dim = c( 939, 939, 3 ) )
  friendship1 <- sienaDependent(FriendsArrayT1)   #Network
  
  SADHD1 <- coCovar( ADHD1[ , 4 ] ) #behavior
  ...
  myeff10 <- includeEffects(myeff10, density, recip, transTrip, inPop) #Structural effects
  myeff10 <- includeEffects(myeff10, simX, altX, egoX, interaction1="SADHD1") #Trait on friendship formation
  myalgorithm10 <- sienaAlgorithmCreate( projname = 'SADHD1', useStdInits = FALSE, nsub=4, n3=2000) #Final Algorithm
  ans10 <- siena07( myalgorithm10, data = mydata10, effects = myeff10, nbrNodes=3, useCluster=T, initC=T)  #Compute
